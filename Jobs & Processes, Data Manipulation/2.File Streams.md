...
Chúng ta sẽ giả định rằng có một **file** tên là **`data.txt`** nó chứa nội dụng, chúng ta quan tâm tới việc sửa đổi nó với nhiều cách rất nhau. **`sed`** sẽ đọc nội dung của **file** đó, sửa đổi nội dung theo hướng mà chúng ta quy định và in ra nội dung đã được sửa đổi.
...
        Chúng ta sẽ không đi sâu vào cú pháp này, nhưng **`[ \t]*$`** là một *regular-expression-ese* đại diện cho "*không hoặc nhiều khoảng trắng hoặc tab nào tới cuồi dòng*".
...
Như bạn đã thấy đó, **`sed`** cung cấp một công cụ mạnh để làm việc với văn bản. Nó là một chung những công cụ phổ biến nhất nếu bạn muốn chuyển đổi đầu ra của một chương trình hoặc nội dung của một **file** theo một số quy định bạn đặt ra. 
...
Lệnh **`awk`** là một công cụ phổ biến dùng để hiển thị hoặc chuyển đổi dữ liệu **text**. Để minh hoạt cho **`awk`**, có rất nhiều các chương trình thao tác với mục đích đặc biệt như **`head`**, **`tail`**, **`sed`** và **`cut`** có thể được triển khai dưới dạng một **script AMK** nhỏ.
